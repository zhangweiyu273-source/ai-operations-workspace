...
def test_knowledge_crud_search_filters_tags_stats_and_soft_delete():
 c,m=client()
 with c:
  a=c.post("/api/v1/knowledge",json=data()).json();c.post("/api/v1/knowledge",json=data("销售异议",category="销售话术",tags=["家长"],priority="中",content="异议处理话术")).json()
...
def test_unified_knowledge_search_covers_tags_category_and_content():
 c,_=client()
 with c:
  tagged=c.post("/api/v1/knowledge",json=data("标签命中",content="不含目标词",summary="普通摘要",tags=["XSC"])).json()
  categorized=c.post("/api/v1/knowledge",json=data("分类命中",category="行业资料",content="不含目标词",summary="普通摘要",tags=["广州"])).json()
  content=c.post("/api/v1/knowledge",json=data("正文命中",content="HD 是活动的家长圈称呼",summary="普通摘要",tags=["术语"])).json()
  assert c.get("/api/v1/knowledge?search=XSC").json()["items"][0]["id"]==tagged["id"]
  assert c.get("/api/v1/knowledge?search=行业资料").json()["items"][0]["id"]==categorized["id"]
  assert c.get("/api/v1/knowledge?search=HD").json()["items"][0]["id"]==content["id"]
