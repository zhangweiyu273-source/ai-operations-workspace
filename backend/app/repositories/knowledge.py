...
class KnowledgeRepository:
 ...
 def filters(self, org: UUID, **p):
  ...
  if p.get("search"):
   search=p["search"]
   values.append(or_(Knowledge.title.contains(search,autoescape=True),Knowledge.content.contains(search,autoescape=True),Knowledge.summary.contains(search,autoescape=True),Knowledge.category.contains(search,autoescape=True),Knowledge.id.in_(select(KnowledgeTag.knowledge_id).where(KnowledgeTag.tag_name.contains(search,autoescape=True)))))
  return values
 ...
 def stats(self,org):
  w=self.filters(org); cutoff=datetime.now(UTC)-timedelta(days=7); return self.session.execute(select(func.count(Knowledge.id),func.count(func.distinct(Knowledge.category)),func.count(Knowledge.id).filter(Knowledge.priority=="高"),func.count(Knowledge.id).filter(Knowledge.updated_at>=cutoff)).where(*w)).one()
